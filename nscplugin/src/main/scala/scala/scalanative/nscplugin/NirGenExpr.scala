...
trait NirGenExpr[G <: nsc.Global with Singleton] { self: NirGenPhase[G] =>
  ...
  class ExprBuffer(implicit fresh: Fresh) extends FixupBuffer { buf =>
    ...
    def genFunction(tree: Function): Val = {
      ...
      val alloc = if (tree.symbol.hasAnnotation(LocalClass)) {
        // Local allocation on the shadow stack
        // reporter.echo(tree.pos, "Local allocation inserted")
        buf.localalloc(anonName, unwind)
      } else {
        buf.classalloc(anonName, unwind)
      }
    ...
    def isStackClass(cls: Symbol): Boolean = {
      (cls :: cls.parentSymbols).exists(_.hasAnnotation(LocalClass))
    }
    ...
    def genApplyNew(app: Apply): Val = {
      ...
      val local = app.getAndRemoveAttachment[DelambdafyTarget.type] match {
        case Some(_) => /* reporter.echo(app.pos, "Local allocation inserted");*/ true
        case None => false
      }

      SimpleType.fromType(tpt.tpe) match {
        case SimpleType(ArrayClass, Seq(targ)) =>
          genApplyNewArray(targ, args, local)

        case st if st.isStruct =>
          genApplyNewStruct(st, args)

        case SimpleType(cls, Seq()) =>
          genApplyNew(cls, fun.symbol, args, local)
    ...
    def genApplyNewArray(targ: SimpleType, argsp: Seq[Tree], local: Boolean = false)(
        implicit pos: nir.Position): Val = {
      val Seq(lengthp) = argsp
      val length       = genExpr(lengthp)

      if (local) {
        buf.localarrayalloc(genType(targ), length, unwind)
      } else {
        buf.arrayalloc(genType(targ), length, unwind)
      }
    }

    def genApplyNew(clssym: Symbol, ctorsym: Symbol, args: List[Tree], local: Boolean = false)(
        implicit pos: nir.Position): Val = {
      val name = genTypeName(clssym)
      val alloc = if (local || isStackClass(clssym)) buf.localalloc(name, unwind)
                  else buf.classalloc(name, unwind)
