...
object Op extends Base[nir.Op] {
  ...
  val Localalloc = P("localalloc" ~ Global.parser map (nir.Op.Localalloc(_)))
  val Fieldload =
  ...
  val Arrayalloc = P("arrayalloc[" ~ Type.parser ~ "]" ~ Val.parser ~ Val.parser map {
    case (ty, init, local) => nir.Op.Arrayalloc(ty, init, local)
  })
  val Arrayload = P(
  ...
  override val parser: P[nir.Op] =
    Call | Load | Store | Elem | Extract | Insert | Stackalloc | Bin | Comp | Conv | Classalloc | Localalloc | Fieldload | Fieldstore | Method | Dynmethod | Module | As | Is | Copy | Sizeof | Box | Unbox | Var | Varload | Varstore | Arrayalloc | Arrayload | Arraystore | Arraylength
}
