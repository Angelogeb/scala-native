...
void Marker_markProgramStack(Heap *heap, Stack *stack) {
    ...
    word_t **shadowStackCurr = (word_t**) local_current;
    shadowStackCurr -= 1;
    word_t **shadowStackBot = (word_t**) local_start;
//
//    printf("%lu\n", shadowStackCurr - shadowStackBot);
//    fflush(stdout);
    while (shadowStackCurr > shadowStackBot) {
        word_t *stackObject = *shadowStackCurr;
        if (Heap_IsWordInHeap(heap, stackObject)) {
            Marker_markConservative(heap, stack, stackObject);
        }
        shadowStackCurr -= 1;
