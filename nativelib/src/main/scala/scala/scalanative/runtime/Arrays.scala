...
object BooleanArray {
  ...
  @inline def alloc(length: Int, local: Boolean = false): BooleanArray = {
    val arrty   = toRawType(classOf[BooleanArray])
    val arrsize = (16 + 1 * length).toULong
    val arr     = if (local) {
      Local.alloc(arrty, arrsize)
    } else { GC.alloc_atomic(arrty, arrsize) }
...
object CharArray {
  ...
  @inline def alloc(length: Int, local: Boolean = false): CharArray = {
    val arrty   = toRawType(classOf[CharArray])
    val arrsize = (16 + 2 * length).toULong
    val arr     = if (local) {
      Local.alloc(arrty, arrsize)
...
object ByteArray {
  ...
  @inline def alloc(length: Int, local: Boolean = false): ByteArray = {
    val arrty   = toRawType(classOf[ByteArray])
    val arrsize = (16 + 1 * length).toULong
    val arr     = if (local) {
      Local.alloc(arrty, arrsize)
...
object ShortArray {
  ...
  @inline def alloc(length: Int, local: Boolean = false): ShortArray = {
    val arrty   = toRawType(classOf[ShortArray])
    val arrsize = (16 + 2 * length).toULong
    val arr     = if (local) {
      Local.alloc(arrty, arrsize)
...
object IntArray {
  ...
  @inline def alloc(length: Int, local: Boolean = false): IntArray = {
    val arrty   = toRawType(classOf[IntArray])
    val arrsize = (16 + 4 * length).toULong
    val arr     = if (local) {
      Local.alloc(arrty, arrsize)
...
object LongArray {
  ...
  @inline def alloc(length: Int, local: Boolean = false): LongArray = {
    val arrty   = toRawType(classOf[LongArray])
    val arrsize = (16 + 8 * length).toULong
    val arr     = if (local) {
      Local.alloc(arrty, arrsize)
...
object FloatArray {
  ...
  @inline def alloc(length: Int, local: Boolean = false): FloatArray = {
    val arrty   = toRawType(classOf[FloatArray])
    val arrsize = (16 + 4 * length).toULong
    val arr     = if (local) {
      Local.alloc(arrty, arrsize)
...
object DoubleArray {
  ...
  @inline def alloc(length: Int, local: Boolean = false): DoubleArray = {
    val arrty   = toRawType(classOf[DoubleArray])
    val arrsize = (16 + 8 * length).toULong
    val arr     = if (local) {
      Local.alloc(arrty, arrsize)
...
object ObjectArray {
  ...
  @inline def alloc(length: Int, local: Boolean = false): ObjectArray = {
    val arrty   = toRawType(classOf[ObjectArray])
    val arrsize = (16 + 8 * length).toULong
    val arr     = if (local) {
      Local.alloc(arrty, arrsize)
    } else { GC.alloc(arrty, arrsize) }
    storeInt(elemRawPtr(arr, 8), length)
    storeInt(elemRawPtr(arr, 12), 8.toInt)
    castRawPtrToObject(arr).asInstanceOf[ObjectArray]
  }
