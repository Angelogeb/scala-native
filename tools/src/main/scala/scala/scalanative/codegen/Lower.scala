...
object Lower {
  ...
  private final class Impl(implicit meta: Metadata) extends Transform {
    ...
    def genOp(buf: Buffer, n: Local, op: Op)(implicit pos: Position): Unit = {
      op match {
        ...
        case op: Op.Localalloc =>
          genLocalallocOp(buf, n, op)
        case op: Op.Conv =>
          genConvOp(buf, n, op)
        case op: Op.Call =>
          genCallOp(buf, n, op)
    ...
    def genLocalallocOp(buf: Buffer, n: Local, op: Op.Localalloc)(
      implicit pos: Position): Unit = {
      val Op.Localalloc(ClassRef(cls)) = op

      val size = MemoryLayout.sizeOf(layout(cls).struct)

      buf.let(
        n,
        Op.Call(allocSig, localAlloc, Seq(rtti(cls).const, Val.Long(size))),
        unwind)
    }
    ...
    def genArrayallocOp(buf: Buffer, n: Local, op: Op.Arrayalloc)(
        implicit pos: Position): Unit = {
      val Op.Arrayalloc(ty, v, local) = op
      ...
      init match {
        case len if len.ty == Type.Int =>
          ...
          buf.let(n,
                  Op.Call(sig, Val.Global(func, Type.Ptr), Seq(module, len, local)),
                  unwind)
  ...
  val localAllocName = extern("scalanative_localalloc")
  val localAlloc = Val.Global(localAllocName, allocSig)
  ...
  val arrayAlloc = Type.typeToArray.map {
    case (ty, arrname) =>
      ...
      ty -> Global.Member(Global.Top(id + "$"),
                          Sig.Method("alloc", Seq(Type.Int, Type.Bool, arrcls)))
  }.toMap
  val arrayAllocSig = Type.typeToArray.map {
    case (ty, arrname) =>
      val Global.Top(id) = arrname
      ty -> Type.Function(Seq(Type.Ref(Global.Top(id + "$")), Type.Int, Type.Bool),
                          Type.Ref(arrname))
  }.toMap
  ...
  val injects: Seq[Defn] = {
    ...
    buf += Defn.Declare(Attrs.None, localAllocName, allocSig)
