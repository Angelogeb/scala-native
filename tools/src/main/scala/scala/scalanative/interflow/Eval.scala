...
trait Eval { self: Interflow =>
  ...
  def eval(op: Op)(implicit state: State,
    ...
    op match {
      ...
      case Op.Localalloc(ClassRef(cls)) =>
        Val.Virtual(state.allocClass(cls, local = true))
      ...
      case Op.Arrayalloc(ty, init, local) =>
        eval(init) match {
          ...
          case Val.ArrayValue(_, values) if values.size <= 128 =>
            ...
            Val.Virtual(addr)
          case init =>
            emit(Op.Arrayalloc(ty, materialize(init), local))
        }
  ...
  private def isPureModule(clsName: Global): Boolean = {
    ...
    def isPureModuleCtor(defn: Defn.Define): Boolean = {
      ...
      defn.insts.foreach {
        case Inst.Let(n, Op.Arrayalloc(_, init, local), _) =>
          canStoreTo += n
          init match {
            case Val.Int(size) =>
              arrayLength(n) = size
        ...
        case Inst.Let(n, _: Op.Classalloc | _: Op.Localalloc | _: Op.Box | _: Op.Module, _) =>
          canStoreTo += n
        case _ =>
          ()
      }
      ...
      defn.insts.forall {
        ...
        case Inst.Let(_, op, _) if op.isPure =>
          true
        case Inst.Let(_, _: Op.Classalloc | _: Op.Localalloc | _: Op.Arrayalloc | _: Op.Box, _) =>
          true
