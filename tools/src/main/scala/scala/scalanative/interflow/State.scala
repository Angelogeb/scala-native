...
final class State(block: Local) {
  ...
  def allocClass(cls: Class, local: Boolean = false): Addr = {
    val fields = cls.fields.map(fld => Val.Zero(fld.ty).canonicalize)
    val kind = if (!local) ClassKind else LocalKind
    alloc(kind, cls, fields.toArray[Val])
  }
  ...
  def materialize(rootValue: Val)(implicit linked: linker.Result,
    ...
    def reachAlloc(addr: Addr): Val = heap(addr) match {
      ...
      case VirtualInstance(LocalKind, cls, _) =>
        emit.localalloc(cls.name, Next.None)
