...
sealed abstract class Op {
  final def resty: Type = this match {
    ...
    case Op.Localalloc(n)           => Type.Ref(n, exact = true, nullable = false)
...
object Op {
  ...
  final case class Localalloc(name: Global)                    extends Op
  ...
  final case class Arrayalloc(ty: Type, init: Val, local: Val = Val.False)         extends Op
