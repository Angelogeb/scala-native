...
class Buffer(implicit fresh: Fresh) {
  ...
  def localalloc(name: Global, unwind: Next)(implicit pos: Position): Val =
    let(Op.Localalloc(name), unwind)
  ...
  def arrayalloc(ty: Type, init: Val, unwind: Next)(
      implicit pos: Position): Val =
    let(Op.Arrayalloc(ty, init, Val.False), unwind)
  def localarrayalloc(ty: Type, init: Val, unwind: Next)(
      implicit pos: Position): Val =
    let(Op.Arrayalloc(ty, init, Val.True), unwind)
