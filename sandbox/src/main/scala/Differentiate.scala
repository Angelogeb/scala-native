...
@stack class StackTuple2[T1, T2](val _1: T1 @stack, val _2: T2 @stack)

@stack class Shift(val fun: ((Tensor @stack => Unit) @stack => Unit) @stack) {
  @inline def map(f: Tensor @stack => Tensor @stack): Shift @stack = {
    new Shift(k => fun(x => k(f(x))))
  }

  @inline def flatMap(f: Tensor @stack => Shift @stack): Shift @stack = {
    new Shift(k => fun(x => f(x).fun(k)))
  }
}
object Shift {
  def reset(c: Shift @stack) = c.fun(_ => ())
}

object Loop {
  @inline def loop(n: Int, body: (Int => Unit) @stack) = {
    var i = 0
    while (i < n) {
      body(i)
      i += 1
...
@stack class Tensor(val shape: List[Int], val data: Array[Float] @stack, val grad: Array[Float] @stack) {
  ...
  @inline def gadd(i: StackTuple2[Integer, Integer] @stack, v: Float) = {
    grad(i._1 * s2 + i._2) += v
  }
...
object Tensor {
  @inline def argmax(t: Tensor @stack) = {
    val a = t.data
    var res = a(0)
    var resi = 0
  ...
  @inline def sum_exp(t: Tensor @stack) = {
    val a = t.data
    var res = 0f
    var i = 0
    val l = t.data.length
  ...
  def matmuladd(W: Tensor, x: Tensor @stack, b: Tensor): Tensor @stack = {
    val rows = W.shape(0)
    val cols = W.shape(1)

    @stack val outA = new Array[Float](rows)
    @stack val outAG = new Array[Float](rows)
  ...
  def `∇matmuladd`(g: Array[Float] @stack, lhs: Tensor, rhs: Tensor @stack, b: Tensor) = {
    val rows = lhs.shape(0)
    val cols = lhs.shape(1)
    loop(rows, r => {
      loop(cols, c => {
        @stack val v1 = new Integer(r)
        @stack val v2 = new Integer(c)
        @stack val idx = new StackTuple2(v1, v2)
...
class Linear(shape: (Int, Int), bias: Boolean = true) {
  ...
  def apply(x: Tensor@stack): Shift @stack = new Shift(
    k => {
...
object Linear {
  def relu(t: Tensor @stack): Shift @stack = new Shift(k => {
    val sz = t.numel
    @stack val outA = new Array[Float](sz)
    @stack val outAG = new Array[Float](sz)
  ...
  def loss(target: Tensor, pred: Tensor @stack): Shift @stack = new Shift(
    k => {
      val cl = Tensor.argmax(target)
      @stack val lossA = new Array[Float](1)
      @stack val lossAG = new Array[Float](1)
  ...
  def grad(f: Tensor => Shift @stack)(x: Tensor) = reset(f(x))
...
class MLP(val shapes: List[(Int, Int)]) {
  ...
  def apply(x: Tensor): Shift @stack = {
    for {
      z1 <- l1(x)
      h1 <- relu(z1)
      z2 <- l2(h1)
