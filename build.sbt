...
addCommandAlias(
  "run-examples",
  Seq(
    "set sandbox / Compile / mainClass := Some(\"Examples\")",
    "sandbox/run",
    "set sandbox / Compile / mainClass := Some(\"Benchmarks\")"
  ).mkString(";")
)
...
lazy val localpluginTests = project
  .in(file("localplugin-tests"))
  .settings(
    libraryDependencies += scalaOrganization.value % "scala-compiler" % scalaVersion.value
  )
  .dependsOn(nativelib, localplugin)
...
lazy val sandbox =
  project
    .in(file("sandbox"))
    .enablePlugins(MyScalaNativePlugin)
    .settings(mainClass in Compile := Some("Benchmarks"))
    .settings(scalacOptions -= "-Xfatal-warnings")
    .settings(scalacOptions += "-P:local:enable")
    .settings(noPublishSettings)
    .dependsOn(localplugin % "plugin", nscplugin % "plugin", allCoreLibs, testInterface % Test)
    .dependsOn(tools)
